...
class Bdd():
    ...
    # ==========================================================================
    # Méthodes pour la partie Inscription
    # ==========================================================================

    def rechercher_inscrit(self, nom_inscrit, prenom_inscrit, classe_inscrit, course_active):
        ...
        resultats = None
        parametres = (nom_inscrit, prenom_inscrit, classe_inscrit, course_active)
        requete_sql = """
        ...
        AND coureurs.prenom = ?
        AND coureurs.classe = ?
        AND coureurs.inscrit = 1
        AND coureurs.id_course_fk = ?;"""
    ...
    def coureurs_inscription(self, parrainage_inscription, nom_inscription, prenom_inscription, classe_inscription, course_active):
        ...
        resultats = None
        parametres = (parrainage_inscription, nom_inscription, prenom_inscription, classe_inscription, course_active)
        requete_sql = """
        UPDATE coureurs
        SET inscrit = 1, parrainage = ?
        WHERE coureurs.nom = ?
        AND coureurs.prenom = ?
        AND coureurs.classe = ?
        AND coureurs.id_course_fk = ?;"""
    ...
    def supprimer_coureur(self, nom_inscrit_supr, prenom_inscrit_supr, classe_inscrit_supr, course_active):
        ...
        resultats = None
        parametres = (nom_inscrit_supr, prenom_inscrit_supr, classe_inscrit_supr, course_active)
        requete_sql = """
        UPDATE coureurs
        SET inscrit = 0
        WHERE coureurs.nom = ?
    ...
    def modifier_coureur(self, parrainage_modification, nom_modification, prenom_modification, classe_modification, course_active):
        """Méthodes pour modifier le parrainage d'un élève de la base de donnée
        @param nom, prenom, classe, parrainage
        @return ...
        """

        if self.debug == True:
            print("Bdd::modifier_coureur()")

        #Modification de l'élève
        resultats = None
        parametres = (parrainage_modification, nom_modification, prenom_modification, classe_modification, course_active)
        requete_sql = """
        UPDATE coureurs
        SET parrainage = ?
        WHERE coureurs.nom = ?
	...
	# ==========================================================================
    # Méthodes temps par classe
    # ==========================================================================

    
    def temps_classe (self, course_active):
        """Méthode pour avoir temps de chaque classe
        @param temps des participant et ID de la course active
        @return temps par classe
        """

        if self.debug == True:
            print("Bdd::temps_classe()")

        resultats = None
        parametres = (course_active,)
        requete_sql = """
            SELECT avg (passages.temps) as moyenne, coureurs.classe
            FROM coureurs
            INNER JOIN passages
    ...
    # ==========================================================================
    # Méthodes pour l'IHM web static
    # ==========================================================================

    def recuperer_passages(self, course_active):
        """Méthodes pour récupérer la liste des participant avec leurs nom / prenom / classe / dernier temps / meilleur temps / nombre de tour / argent récoltés
        @param course active
        @return tableau de tuples
        """

        if self.debug == True:
            print("Bdd::recuperer_passages()")

        #récupération des données
        resultats = None
        parametres = (course_active,)
        requete_sql = """
            SELECT DISTINCT(coureurs.id), coureurs.nom, coureurs.prenom, coureurs.classe, coureurs.meilleur_temps, coureurs.distance_totale, coureurs.parrainage
            FROM coureurs
            INNER JOIN passages
            ON coureurs.id = passages.id_coureur_fk
            WHERE coureurs.id_course_fk = ?
            ...
            GROUP BY coureurs.id, coureurs.nom, coureurs.prenom, coureurs.classe, coureurs.dernier_temps, coureurs.meilleur_temps, coureurs.parrainage, coureurs.distance_totale
            ORDER BY coureurs.distance_totale DESC, coureurs.meilleur_temps ASC;"""
    ...
    # ==========================================================================
    # Méthodes pour la partie autocomplete
    # ==========================================================================

    def autocomplete(self, course_inscrit, course_active):
        """Méthodes pour récupérer la liste des classe, nom et prénom des participant
        @param course inscrit et course active
        @return tableau de tuples (nom, prenom, classe)
        """
        if self.debug == True:
            print("Bdd:autocomplete()")

        #récupération des données
        resultats = None
        parametres = (course_inscrit, course_active)
        requete_sql = """
    ...
    def autocomplete_recherche(self, course_active):
        """Méthodes pour récupérer la liste des classe, nom et prénom des participant
        @param course active
        @return tableau de tuples (nom, prenom, classe)
        """
        if self.debug == True:
            print("Bdd:autocomplete_recherche()")
            ...
            with open(fichier_csv, newline='') as csvfile:
                ...
                for ligne in coureurs:
                    ...
                    requete_sql = """
                        ...
                        (nom, prenom, classe, code_barre, parrainage, inscrit, id_course_fk)
                        VALUES (?, ?, ?, ?, ?, ?, ?);"""
...
if __name__ == "__main__":
    ...
    try:
        ...
        # AJout d'une course
        #print("Bdd::ajouter_une_course()")
        #print(bdd.ajouter_une_course("0000-00-00", "test"))
        ...
        #print("Bdd::ajouter_coureurs_depuis_csv()")
        #bdd.ajouter_coureurs_depuis_csv("./static/uploads/coureurs_badge.csv", 3)

        # Affichage de la course active
        #print("Bdd::reinitialiser_les_passages()")
        #print(bdd.reinitialiser_les_passages(3))

        # Recuperer les courses récente
        #print("Bdd::recuperer_courses_recentes()")
        #print(bdd.recuperer_courses_recentes)

        # Inscription des coureurs
        #print("Bdd::coureurs_inscription()")
        #print(bdd.coureurs_inscription(90, "CAME", "LEON", "SEGPA", 3))

        # Modification d'un coureur
        #print("Bdd::modifier_coureur()")
        #print(bdd.modifier_coureur( 5, "CAME", "LEON", "SEGPA", 3))

        # Recherche d'un coureur
        #print("Bdd::modifier_coureur()")
        #print(bdd.rechercher_inscrit("BEAUNNO", "JEAN", "SNIR3", 1))

        # Autocomplete
        #print("Bdd::autocomplete()")
        #print(bdd.autocomplete(0, 3))

        # Autocomplete
        #print("Bdd::autocomplete_recherche()")
        #print(bdd.autocomplete_recherche(3))

        # recuperer_passage
        print("Bdd::recuperer_passages()")
        print(bdd.recuperer_passages(3))
