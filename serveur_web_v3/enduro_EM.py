...
def afficher_resultats():
	...
	try:
		...
		informations["erreur"] = ""
		moyenne_classe = bdd.temps_classe(session["id_course_active"])
	...
	if moyenne_classe and len(moyenne_classe[0]) == 2:
		informations["classe"] = moyenne_classe[0][1]
		informations["classe_temps"] = formater_temps(moyenne_classe[0][0])
	else:
		informations["classe"] = "???"
		informations["classe_temps"] = "??:??"

	try:
		coureurs = bdd.recuperer_passages(session["id_course_active"])
		print(coureurs)

	except ErreurBdd as erreur:
		flash("Une erreur est survenue (%s)" % erreur.message, "danger")

	return render_template('public_resultats.html', informations=informations, coureurs=coureurs)
...
def afficher_inscription():
	...
	try:
		...
		languages = bdd.autocomplete_recherche(course_active)
		dernieres_stats = bdd.recuperer_dernieres_statistiques(course_active)
		nb_coureurs = dernieres_stats[0][2]
	except:
		flash("Une erreur est survenue (%s)" % erreur, "danger")

	return render_template('public_inscription.html', languages=languages, nb_coureurs=nb_coureurs)

@app.route('/inscription/modification', methods=["GET"])
def afficher_inscription_modification():
	...
	try:
		course = bdd.recuperer_course_active()
		course_active = course[0][0]
		languages = bdd.autocomplete(1, course_active)
		print(languages)
	except:
		flash("Une erreur est survenue (%s)" % erreur, "danger")

	return render_template('public_inscription_etape3.html', languages=languages)

@app.route('/inscription/suppression', methods=["GET"])
def afficher_inscription_suppression():
	...
	return render_template('public_inscription_etape4.html', languages=languages)
...
@app.route('/inscription/recherche', methods=["POST", "GET"])
def recherche_inscrit():
	...
	return render_template('public_inscription_etape2.html', deja_inscrit=deja_inscrit, languages=languages, nb_coureurs=nb_coureurs)
...
def inscription():
	...
	try:
		course = bdd.recuperer_course_active()
		course_active = course[0][0]
		languages = bdd.autocomplete(0, course_active)
		dernieres_stats = bdd.recuperer_dernieres_statistiques(course_active)
		nb_coureurs = dernieres_stats[0][2]
	except ErreurBdd as erreur:
		flash("Une erreur est survenue (%s)" % erreur, "danger")
	...
	#Envoie de la requête SQL
	try:
		coureurs_inscription_en_cours = bdd.coureurs_inscription(parrainage_inscription, nom_inscription, prenom_inscription, classe_inscription, course_active)
	...
	return render_template('public_inscription_etape2.html', languages=languages, nb_coureurs=nb_coureurs)
